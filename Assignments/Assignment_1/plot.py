...
def plot_costs(module_name, nb_of_epochs, nb_of_batches, minibatch_costs, train_costs, val_costs, test_costs):

    minibatch_x = np.linspace(0, nb_of_epochs, num=nb_of_epochs * nb_of_batches)
    epochs_x = np.linspace(1, nb_of_epochs, num=nb_of_epochs)
    # plt.plot(minibatch_x, minibatch_costs, 'k-', linewidth=0.5, label='Minibatches', alpha=0.5)
